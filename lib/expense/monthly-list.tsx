...
const MonthlyExpenseList: FC = () => {
  ...
  function createdAt(expense: Expense) {
    return DateTime.fromJSDate(expense.date).toFormat("LLLL yyyy")
  }

  const monthlyExpenses: MonthlyExpenses = pipe([groupBy(createdAt), mapValues(groupBy("cat"))])(
    expenses,
  )
